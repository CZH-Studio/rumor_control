...
class Platform:
    ...
    async def running(self):
        while True:
            message_id, data = await self.channel.receive_from() #通过channel监听消息
            ...
            action_function = getattr(self, action.value, None) #通过action从自身获取对应函数（下面那些），默认None
            if action_function:
                # Get the names of the parameters of the function
                func_code = action_function.__code__ #解析函数代码
                param_names = func_code.co_varnames[:func_code.co_argcount] #获取函数所有变量
                ...
                result = await action_function(**params) #调用自己的函数（下面的那些）
    ...
    async def refresh(self, agent_id: int, post_id: int):
        ...
        try:
            ...
            if self.recsys_type != RecsysType.REDDIT:
                ...
                if post_id != -1:
                    selected_post_ids = [post_id] + following_posts_ids + selected_post_ids
                else:
    ...
    async def fetch_post(self, agent_id: int):
        ...
        try:
            post_query = (
                f"SELECT post_id, user_id, original_post_id, content, "
                f"quote_content, created_at, num_likes, num_dislikes, "
                f"num_shares FROM post WHERE post_id = ?")
            self.pl_utils._execute_db_command(post_query, (agent_id, ))
            results = self.db_cursor.fetchall()
            results_with_comments = self.pl_utils._add_comments_to_posts(
                results)
    ...
    async def like_post(self, agent_id: int, post_id: int):
        ...
        try:
            post_type_result = self.pl_utils._get_post_type(post_id)
            if post_type_result['type'] == 'repost': #如果只是转发推，那么点赞算到原推文身上
                post_id = post_type_result['root_post_id']
            user_id = agent_id
            # Check if a like record already exists（防止重复点赞）
            ...
            self.pl_utils._record_trace(user_id, ActionType.LIKE_POST.value, #日志插入数据库
                                        action_info, current_time)
