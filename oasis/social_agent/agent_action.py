...
class SocialAction:
    ...
    async def perform_action(self, message: Any, type: str):
        message_id = await self.channel.write_to_receive_queue( #放入接收队列，等待被DB捕获执行
            (self.agent_id, message, type))
        response = await self.channel.read_from_send_queue(message_id) #从发送队列中读取执行结果
        return response[2] #返回(message_id, agent_id, result)中的result
    ...
    async def refresh(self, post_id: int):
        ...
        Args:
            post_id (int): only used when vaccined.
        ...
        """
        return await self.perform_action(post_id, ActionType.REFRESH.value)
